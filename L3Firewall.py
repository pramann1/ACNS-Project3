...
class Firewall (EventMixin):
	...
	def allowOther(self,event):
		...
		match = of.ofp_match()
                ...
                print " Packet.dst in allowOther = ", str(packet.dst)
                print "Source mac in allowOther =", str(packet.src)
                print "Source IP in allowOther =", str(match1.nw_src)
                print "self.macToip.get(str(packet.src) =", self.macToip.get(str(packet.src))
                if (str(match1.nw_src) == self.macToip.get(str(packet.src)) and 
                   str(packet.src) == self.ipTomac.get(str(match1.nw_src))):
                    print "Inside if /n"
	...
	def _handle_PacketIn(self, event):
		...
		match = of.ofp_match.from_packet(packet)
                #self.macToPort [str(packet.src)] = str(match.nw_src)
                #print "/n" 
                #print "Packet src in handlePacket = ", str(packet.src)
                #print "Source IP in handlePacket = ", str(match.nw_src)
                #print "Destination in han= ", str(match.nw_dst)
                #print "Destination = ", str(match.dl_dst)
		if(match.dl_type == packet.ARP_TYPE and match.nw_proto == arp.REQUEST):
                  print "/n"
                  print "Packet src in handlePacket = ", str(packet.src)
                  print "Source IP in handlePacket = ", str(match.nw_src)
                  self.macToip [str(packet.src)] = str(match.nw_src)
                  self.ipTomac [str(match.nw_src)] = str(packet.src)
		  self.replyToARP(packet, match, event)

		if(match.dl_type == packet.IP_TYPE):
		  ip_packet = packet.payload
                  #print "Source mac = ", str(match.dl_src)
                  #print "Destination mac = ", str(match.dl_dst)
                  #print "Source IP = ", str(match.nw_src)
                  #print "Dest IP = ", str(match.nw_dst)
